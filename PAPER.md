...
4. **Seven theorems (F–L)**, each with numerical verification
   (`src/theory/theorems.py`, exercised by `tests/`), including **Theorem I**
...
**FuzzyInferenceEngine** (`src/reasoning/abstract_inference.py`) represents
...
### 3.1 Multi-hop inference — a Pareto point (`src/experiments/inference_eval.py`)
...
### 3.2 Operator algebra — Theorem G (`src/reasoning/operator_algebra.py`)
...
### 3.3 Spectral analysis — Theorem H (`src/reasoning/relation_spectrum.py`)
...
### 4.1 Blocking hallucination — `src/experiments/guard_llm_eval.py`
...
### 4.2 The hallucination boundary — `src/experiments/nl_ontology_eval.py`
...
### 4.3 Token cost — `src/experiments/guard_cost_eval.py`
...
### 4.4 A public benchmark — CLUTRR (`src/experiments/clutrr_eval.py`)
...
external knowledge and no extra tokens (`src/experiments/self_grounded_eval.py`).
...
**End-to-end demo on a real LLM** (`src/experiments/conformal_llm_eval.py`).
...
- Engine: `src/reasoning/{abstract_inference,operator_algebra,relation_spectrum}.py`
- LLM client (key read from an environment variable): `src/reasoning/llm_client.py`
- Real-LLM experiments: `src/experiments/{guard_llm_eval,nl_ontology_eval,guard_cost_eval,inference_eval,clutrr_eval,conformal_llm_eval,self_grounded_eval}.py`
- Seven theorems (F–L): `src/theory/theorems.py`, exercised by `tests/test_theorems.py`
