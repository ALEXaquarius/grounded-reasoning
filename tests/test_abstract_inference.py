"""
Tests cho thuật toán suy diễn trừu tượng (FuzzyInferenceEngine) + Định lý F.
"""
...
class TestFuzzyInference:
    def test_deep_chaining(self):
        e = _chain_engine()
        # suy ra a→e (4 bước) dù không có cạnh trực tiếp
        assert e.confidence("a", "e") > 0

    def test_confidence_decays_with_depth(self):
        e = _chain_engine()
        c = {b: e.confidence("a", b) for b in ["b", "c", "d", "e"]}
        assert c["b"] > c["c"] > c["d"] > c["e"] > 0  # xa hơn = kém chắc

    def test_grounded_no_hallucination(self):
        e = _chain_engine()
        e.add_relation("x", "y")  # cụm rời
        assert e.confidence("a", "y") == 0.0   # không suy ra quan hệ không có đường
        assert e.confidence("y", "a") == 0.0

    def test_explain_returns_path(self):
        e = _chain_engine()
        assert e.explain("a", "e") == ["a", "b", "c", "d", "e"]
        assert e.explain("a", "z") is None      # không bịa đường

    def test_directional(self):
        e = _chain_engine()
        assert e.confidence("a", "e") > 0
        assert e.confidence("e", "a") == 0.0    # quan hệ có hướng
...
class TestTypedInference:
    ...
    def test_compositional_grandparent(self):
        ...
        assert e.follow("eve", ["parent", "parent", "parent"]) == {"al"}  # 3-hợp thành

    def test_analogy(self):
        e = self._kin()
        # dave:carol :: eve:? → dave (cùng quan hệ parent)
...
class TestHallucinationGuard:
    def test_guard_perfect_on_grounded_facts(self):
        ...
        ok, path = g.verify("a", "d")       # thật (bắc cầu)
        assert ok and path == ["a", "b", "c", "d"]
        ok2, path2 = g.verify("a", "z")     # bịa
        assert not ok2 and path2 is None
        ok3, _ = g.verify("d", "a")         # sai hướng (ảo giác)
        assert not ok3
...
class TestInferenceAB:
    def test_fuzzy_pareto_dominates(self):
        r = evaluate(seed=0)
        # Fuzzy: vừa suy sâu 100% VỪA không ảo giác — điểm Pareto không cách nào khác đạt
        assert r["B_fuzzy"]["deep_recall"] == 1.0
        assert r["B_fuzzy"]["halluc_rate"] == 0.0
        # one-hop không ảo giác nhưng không suy được chuỗi sâu
        assert r["A_one_hop"]["deep_recall"] == 0.0
        assert r["A_one_hop"]["halluc_rate"] == 0.0
        # guesser phủ cao nhưng ảo giác nặng
