...
cách này **không có bảo đảm chống nhầm** — đánh đổi thật, đã ĐO được chứ
không chỉ nêu khả năng: với cách chia 50/50 dữ liệu giữ riêng (nửa để tìm
cạnh khả nghi, nửa để đánh giá) và luật mặc định, **17–19% cạnh bị xóa
thực ra là cạnh đúng**. Dùng phần lớn hơn dữ liệu giữ riêng để tìm cạnh
khả nghi (`identify_frac=0.8` thay vì 0.5) giảm còn **~5.5%**, và thêm yêu
cầu một claim sai thứ hai xác nhận cùng cạnh (`min_evidence=2`) giảm tiếp
còn **~4.2–4.5%** — đổi lại là tập đánh giá giữ riêng nhỏ hơn và FPR sau
khi làm sạch cao hơn một chút (~58.0% thay vì ~49.2% ở cùng mức nhiễu, vẫn
thấp hơn nhiều so với 77.0% ban đầu). Cách này vẫn luôn tốn recall thật với
