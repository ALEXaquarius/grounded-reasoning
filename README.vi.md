...
`::TestTransitiveRelationsGuard`, `::TestTransitivityCalibration`; so sánh A/B
với guard nhị phân:
...
Năm định lý mở rộng thêm: **I** (bảo đảm hai chiều precision/recall cho biến thể
...
Horn, tổng quát hóa bao đóng bắc cầu sang luật có nhiều tiền đề), và **M** (hiệu
chỉnh thực nghiệm giả định bắc cầu — cận tin cậy Clopper-Pearson thay thế một giả
định bắc cầu mù bằng một giả định đo được). Cả tám định lý được
...
| `grounded_reasoning/reasoning/transitivity_calibration.py` | Hiệu chỉnh Clopper-Pearson cho giả định bắc cầu (Định lý M) |
| `grounded_reasoning/reasoning/llm_client.py` | LLM client không phụ thuộc nhà cung cấp (key đọc từ biến môi trường) |
| `grounded_reasoning/theory/theorems.py` | **Tám định lý (F–M)** với kiểm chứng số |
