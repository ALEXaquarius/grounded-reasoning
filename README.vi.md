...
solver phủ 99.5%, accuracy 99.2%. `src/experiments/clutrr_eval.py`.)*
...
⟹ suy diễn mờ **chính là** phân tích phổ của toán tử quan hệ. `src/reasoning/`.
...
cứng không chạm tới được. `src/experiments/conformal_llm_eval.py`.
...
```bash
pip install -e ".[dev]"             # hoặc: pip install grounded-reasoning
...
python -m src.experiments.guard_llm_eval        # guard chống ảo giác
python -m src.experiments.self_grounded_eval    # SGDC
python -m src.experiments.clutrr_eval           # benchmark công khai CLUTRR
python -m src.experiments.conformal_llm_eval    # conformal đầu-cuối (đồ thị do LLM trích)
```

---

## Tích hợp với Agent / LLM (`src/agent/`)
...
- **MCP server**: `python -m src.agent.mcp_server` — cắm vào Claude hoặc bất kỳ agent tương thích MCP nào.
...
| `src/agent/{verifier,tool,mcp_server}.py` | Triển khai API công khai — HallucinationGuard, tool function-calling, MCP server |
| `src/reasoning/abstract_inference.py` | FuzzyInferenceEngine, TypedInferenceEngine, HallucinationGuard (Định lý F) |
| `src/reasoning/operator_algebra.py` | Đại số toán tử quan hệ (Định lý G) |
| `src/reasoning/relation_spectrum.py` | Phổ, tính lũy linh, giải thức Katz (Định lý H) |
| `src/reasoning/conformal_reasoning.py` | Conformal — bảo đảm coverage dưới nhiễu (Định lý K) |
| `src/reasoning/composition_algebra.py` | Học bảng hợp thành, kiểm chứng trên CLUTRR (Định lý J) |
| `src/reasoning/horn.py` | Suy diễn tiến Horn, ngữ nghĩa least-model (Định lý L) |
| `src/theory/theorems.py` | **Bảy định lý (F–L)** với kiểm chứng số |
| `src/experiments/*.py` | Thực nghiệm LLM thật và benchmark hậu thuẫn mọi khẳng định trên |
